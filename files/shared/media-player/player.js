...
$(this).find("video").css({"object-fit":"contain"});
}
}
...
}else{ var media_play_list=""; var media_play_next=""; var media_play_prev=""; }
...
$(this).find("video").css({"max-height":""+$(this).height()+"px"});
...
if(typeof bar_hide !== 'undefined'){ clearTimeout(bar_hide); }
    $(".media-player video").css({"cursor":""});
    $(".media-meta").show();
bar_hide = setTimeout(function(){
    if(!this_media.paused && this_div.find(".media-ads").length==0){
    $(".media-player video").css({"cursor":"none"});
...
media_type="video";
this_div.find("video").css({"object-fit":"contain"});
}
...
$(this).find(".media-progress-bar").css({"height":"15px"});

}).mouseleave(function() {
$(this).find(".media-progress-bar").css({"height":"6px"});
});
// </Hover seek bar>
...
if(pos<81 || margin_right<81){
	this_div.find(".media-time-hover").show().removeClass("hover-left hover-right").addClass("hover-"+(pos<81 ? "left" : "right")+"").css({"left":""}).text(media_time("",thistime));
	}else{	this_div.find(".media-time-hover").show().removeClass("hover-left hover-right").css({"left":progress_pos+"%"}).text(media_time("",thistime)); }
...
if(progress_pos>100){var progress_pos=100;}
if(progress_pos<0){var progress_pos=0;}
...
if(get_volume_level>=9){var get_volume_level=10;}
$(this).children("span").css({"width":""+get_volume_level*10+"%"});

if(get_volume_level>=9){var volume_level=1.0;}else{var volume_level="0."+get_volume_level+"";}
...
if(type.length==0){ var media_time=" "+(media_current["h"]>0 ? ""+media_current["h"]+":" : "")+""+media_current["m"]+":"+media_current["i"]+" "; }else{var media_time=media_current[type];}
}else{ var media_time="00";}
...
this_div.find(".media-buffered").css({"width":""+last_buffered_bar+"%"});
...
if(this_media.duration>1){ this_div.find(".media-duration-time").text(""+(media_duration["h"]>0 ? ""+media_duration["h"]+":" : "")+""+media_duration["m"]+":"+media_duration["i"]+"");}else{
	setTimeout(function(){
update_currentTime();
}, 2000);
}
...
// <Play or pause>
 ...
 if(media_type=="video"){
	...
	play_warning=setTimeout(function(){ this_div.find(".media-play-warning").show().fadeOut(10000); },15000);
	}
	...
	apply_subtitle();
	if(media_type=="audio"){
	this_div.find(".media-meta img").addClass("img-spin");}else{
	this_div.find(".media-play-btn").removeClass("fa-play-circle-o");	
	}
            }else if(this_media.duration>0){
    this_media.pause();
	this_div.find(".media-play-pause").removeClass("fa-pause").addClass("fa-play");
	if(media_type=="audio"){this_div.find("img").removeClass("img-spin");}else{
...
this_media.addEventListener('loadedmetadata', function(){ this_media.currentTime=toTime; }, false);
...
if (godl) {godl.focus();} else {alert('Vui lòng cho phép mở trong tab mới');}
}
}
...
// <Count>
function media_count(action,type,title){
	
$.ajax({ url: player_path, timeout:2000, type: 'POST', data: {"action":action,"type":type, "title":title} });
}
if(clicked_id!=this_div.index(".media-player") && this_media.currentTime===0){ media_count("media_count","",""); }
// </Count>
...
var ads_runing=0;
var ads_stt = false;
function ads_start(title, video, title_link, url_link, skip_time, audio){
...
if (!ads_stt){ads_stt=true; ads_runing=1; ads_stt=false;}

currentTime = this_media.currentTime; 
change_media(video,0,"");
if(this_div.find("track").length>0){ this_media.textTracks[0].mode = 'hidden'; }
this_div.find(".media-controls").hide().after('<div class="media-ads"> '+(title_link.length>0 ? '<span class="media-ads-link"><a target="_blank" href="'+url_link+'">'+title_link+'</a></span>' : '')+' <span class="media-ads-time"></span> <span style="display:none" class="media-ads-skip">Bỏ Qua &raquo;</span> </div>');
...
var skip_down = setInterval(function() {
    ...
    if (skip_time_count == 0) {  clearInterval(skip_down);  }
}, 1000);

setTimeout(function(){ this_div.find(".media-ads-time").remove(); $(".media-ads-skip").show(); },skip_time*1000);
...
// Skip AD
	...
	if(ads_time<1){ disable_ads(); this_div.find(""+media_type+"").off("timeupdate"); }
	 
    });

$(".media-ads-skip").on("click", function() { disable_ads(); this_div.find(""+media_type+"").off("timeupdate");});
...
media_count("ads_count","video",title);
}
...
$.each(ads_config["video"], function(aid,ads) {
setTimeout(function(){  if(ads_runing==0){  ads_start(ads["title"], ads["video"], ads["title_link"], ads["url_link"], ads["skip_time"], ads["audio"]); }  },ads["sec"]*1000);
});
...
// Close AD
setTimeout(function(){  $(".media-ads-popup").remove();   },hidden_after*1000);
$(".media-ads-close").on("click",function(){ $(".media-ads-popup").remove(); });

$(".ads-click").on("click",function(){ 
$(".media-ads-popup").remove();
media_count("ads_count","popup",title);
...
$.each(ads_config["popup"], function(aid,ads) {
setTimeout(function(){   pop_ads_start(ads["title"],ads["image"],ads["link"],ads["hidden_after"],ads["audio"]);   },ads["sec"]*1000);
});
...
if(this_div.find(".media-play-list .item-active").next().attr("media") !== undefined){
	...
	if(this_div.find(".media-play-list .item-active").next().attr("media") === undefined){ this_div.find(".play-next").hide(); }
	update_quality();
}else{
...
// <Play next>
	if(this_action=="play_prev_next"){
        ...
        if($(e.target).attr("play")=="next"){ this_div.find(".play-prev").show(); }else{ this_div.find(".play-next").show(); }
		change_media(this_div.find(".media-play-list .item-active").attr("media"),0,this_div.find(".media-play-list .item-active").attr("type"));
	    update_quality();
	}
// </Play next>
...
if(this_div.find(".media-subtitle .item-active").length==0){this_div.find(".media-subtitle").parent().hide();}else{this_div.find(".media-subtitle").parent().show();}
apply_subtitle();
...
var media_quality="";
if(this_media.videoHeight>100){var media_quality=" (144P)";}
if(this_media.videoHeight>200){var media_quality=" (240P)";}
if(this_media.videoHeight>300){var media_quality=" (360P)";}
if(this_media.videoHeight>400){var media_quality=" (480P)";}

if(this_media.videoHeight>700){var media_quality=" (HD)";}
if(this_media.videoHeight>1000){var media_quality=" (FHD)";}
if(this_media.videoHeight>1400){var media_quality=" (QHD)";}
if(this_media.videoHeight>2000){var media_quality=" (4K)";}
if(this_media.videoHeight>4000){var media_quality=" (8K)";}
if(this_media.videoHeight>5000){var media_quality=" (++)";}
...
var all_source_media="";
var all_playlist_media="";
$(document).find(".media-player source").each(function(){
	all_source_media+=""+$(this).prop("src")+"\n";
});
$(document).find(".media-player li data").each(function(){
	var this_link=$(this).attr("value")
	if(this_link.charAt(0)=="/" || this_link.indexOf('://') > -1){var before_link="";}else{var before_link=location.href;}
	all_playlist_media+=""+before_link+""+this_link+"\n";
	
});



$.ajax({ url: player_path,timeout:2000, type: 'POST', data: {"action":"save_link_media","link":""+all_source_media+""+all_playlist_media+""} });
...
player_path="/shared/media-player/manager/";
ads_config = {"video": [{
  'sec'       :  '10',
  'skip_time' :  '5',
  'title'     :  '[Quảng cáo] Điện máy xanh',
  'video'     :  'assets/ads-1.mp4',
  'title_link':  'Mua ngay',
  'url_link'  :  'http://dienmayxanh.com',
  'audio'     :  '1'
},{
  'sec'       :  '30',
  'skip_time' :  '10',
  'title'     :  '[Quảng cáo] Kẹo béo',
  'video'     :  'assets/ads-2.mp4',
  'title_link':  '',
  'url_link'  :  '',
  'audio'     :  '0'
},],"popup": [{
  ...
  'image'     :  'http://thuvienanhdep.net/wp-content/uploads/2016/10/hinh-anh-dep-nhat-the-gioi-ve-su-hoan-hao-khien-moi-nguoi-ngo-ngang2.jpg',
  'audio'     :  '1',
},]};
