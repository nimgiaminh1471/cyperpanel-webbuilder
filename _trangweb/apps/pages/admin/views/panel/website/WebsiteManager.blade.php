...
@if( file_get_contents_curl($wLink."/builder/active.txt") != 1 )
	@php
		$pendingSetup = true;
				...
				<div class="website-info">
					@foreach($webInfo as $key=>$item)
						...
						@switch($key)
							...
							@break
						@endswitch
						<div class="flex" style="color: {!! (empty($itemColor) ? "initial" : $itemColor) !!}">
					...
					@endforeach
					@if( isset($pendingSetup) )
						{{-- Website chưa sẵn sàng --}}
					...
					@else
						{{-- Cập nhật web mẫu --}}
						@if( $w->app_price > 0 && !empty($userLogin) )
							...
							<script type="text/javascript">
								function autoUpdateTemplate(){
									...
									$.ajax({
										...
										type: "POST",
										dataType: "JSON",
										data: {onlyDatabase: true, id: {{ $w->id }}},
										success: function(){
						...
						@endif
						{{-- Đi tới trang quản lý web --}}
						<div class="website-goto bg">
							<div class="flex flex-middle">
								<div class="width-33x pd-5">
									<div class="center">
										@if( permission("website_template_master") )
											<a target="_blank" href="{{($w->ssl_type==0 ? 'http://' : 'https://')}}{{$w->domain}}?_is_builder_admin={{user("check_admin")}}&auto_login_by_user={{ urlencode($userLogin ?? null) }}&go_to_dashboard">
										@else
											<a target="_blank" href="{{($w->ssl_type==0 ? 'http://' : 'https://')}}{{ $w->domain }}/admin/?auto_login_by_user={{ urlencode($userLogin ?? null) }}&_login_key={{ md5($userPassword ?? null) }}&go_to_dashboard">
										@endif
											<span>
								...
								<div class="width-33x pd-5">
									<div class="center">
										@if( permission("website_template_master") )
											<a target="_blank" href="{{($w->ssl_type==0 ? 'http://' : 'https://')}}{{$w->domain}}?_is_builder_admin={{user("check_admin")}}&auto_login_by_user={{ urlencode($userLogin ?? null) }}">
										@else
											<a target="_blank" href="{{($w->ssl_type==0 ? 'http://' : 'https://')}}{{ $w->domain }}/admin/?auto_login_by_user={{ urlencode($userLogin ?? null) }}&_login_key={{ md5($userPassword ?? null) }}">
										@endif
											<span>
...
" class="width-100" rows="8" name="website[ssl_cacert]">'.$w->ssl_cacert.'</textarea>
		...
		@if(  empty($w->app_name) || empty($w->app)  )
			<div style="margin-top: 20px">
				...
				<div class="pd-10 bg">
					<div class="flex menu">
						...
						<div style="width: calc(100% - 150px)">
							<b>{{ date("H:i - d/m/Y", timestamp($w->created_at) ) }}</b>
						</div>
					</div>
					<div class="flex flex-middle menu">
						...
						<div style="width: calc(100% - 150px); color: {{$itemColor}}">
							@if( permission('admin', $w->users_id) )
								@if( $w->expired > 0 )
									<div><b>{{ date("H:i - d/m/Y", ($w->expired) ) }}</b></div>
								@endif
								<div class="blue">Website của admin sẽ không tự động xóa</div>
							@else
								<div><b>{{ date("H:i - d/m/Y", ($w->expired) ) }}</b> (<span{!! $expiredDays < 1 ? ' style="color: red"' : '' !!}>Còn <b>{{ $expiredDays }}</b> ngày</span>)</div>
								@if( $expiredDays <= 0 )
									<div>Website sẽ bị xóa vĩnh viễn vào ngày {{ date("d/m/Y", $w->expired + (3600 * 24 * WEB_BUILDER["expired_delete_delay"]) ) }}</div>
								@elseif( $expiredDays<31 )
					...
					<div class="flex menu">
						...
						<div style="width: calc(100% - 150px)">
							<a href="/bang-gia" target="_blank">
								<b>{{ WEB_BUILDER["package"][$w->package]["name"] ?? 'Free' }}</b>
							</a>
						</div>
					</div>
					@if($w->renew_price > 0)
						<div class="flex menu">
							...
							<div style="width: calc(100% - 150px)">
								<b>{{ number_format($w->renew_price) }}</b>/năm
							</div>
						</div>
					@elseif( !empty($w->package) )
						@php
					...
					<div class="menu">
						@if( permission("website_manager") )
							<button data-modal="fa-calendar{{$w->id}}" class="btn btn-info center modal-click">
								<i class="fa fa-calendar"></i> Thay đổi gói & hạn website
							</button>
						@else
							<button data-modal="fa-credit-card{{$w->id}}" class="btn btn-info center modal-click">
		...
		@if( permission("website_manager") )
			...
			<div style="margin-top: 20px">
				...
				<div class="pd-10 bg">
					<div class="menu">
						<i class="fa-icon fa fa-user"></i> {!! user("name_color", $w->users_id) !!}
					</div>
					<div class="menu">
						<i class="fa-icon fa fa-envelope"></i> {{ user("email", $w->users_id) }}
					</div>
					<div class="menu">
						<i class="fa-icon fa fa-phone"></i> {{ user("phone", $w->users_id) }}
					</div>
					<div class="menu">
						...
						<a target="_blank" href="/admin/PaymentHistory?uid={{ $w->users_id }}" class="btn btn-success">
			...
			{!! modal("website-change-user", "Đổi người quản lý", '
	...
	<section class="width-40 flex-margin">
		...
		<div>
			...
			@foreach($webAction as $action=>$item)
				{!! modal($item["icon"]."".$w->id, $item["title"], '
		...
		<div style="margin-top: 20px">
			...
			<div class="menu">
				...
				<div class="center">
					@if( permission('website_manager') )
						<button data-modal="fa-floppy-o{{$w->id}}" class="btn btn-info center modal-click">
						<i class="fa fa-floppy-o"></i> Đổi dung lượng
					</button>
					@else
						<button data-modal="fa-plus{{$w->id}}" class="btn btn-info center modal-click">
			...
			@if( $upgradeDisk > 0 )
				<div class="alert-info">
					Dung lượng đã mua thêm: <b>{{ $upgradeDisk }}</b> Mb
				</div>
			@endif
		</div>
		@if( !isset($pendingSetup) )
				...
				<section style="margin: 0">
					...
					<div class="flex flex-medium">
						<div class="pd-5 width-100">
							<div class="pd-5">
								<a class="btn-primary width-100 link" onclick="downloadWebsiteBackup({{ $w->id }})">Xuất bản website</a>
							</div>
						</div>
					</div>
					<form method="POST">
						<div class="pd-5">
							<textarea rows="5" class="width-100" placeholder="Nhập nội dung để đình chỉ web" name="edit_web[suspended]">{{webConfig($w->domain, "BUILDER_SUSPENDED")}}</textarea>
						</div>
