	...
	<form method="POST" class="width-100 flex-margin" id="cash_flow-form" style="margin-bottom: 5px; min-height: 350px">
		@php
			...
			if( empty($filter["search"]) ){
				...
				// Có tìm kiếm
				foreach(["id", "name", "phone", "email"] as $findColumn){
					$getCashFlow = $getCashFlow->orWhere("users.{$findColumn}", "like", "%{$filter["search"]}%")->whereNull($whereNull)
			...
			<div class="section-body" style="margin-bottom: 10px">
				<div class="flex flex-medium">
					<div class="width-15 pd-5">
						<input type="text" name="filter[search]" placeholder="Tìm theo Email, tên người nạp" class="width-100">
					</div>
					...
					<div class="width-15 pd-5">
						<select name="filter[status]" class="width-100">
					...
					<div class="width-15 pd-5">
						<select name="filter[category_id]" class="width-100">
					...
					<div class="width-15 pd-5">
						<select name="filter[creator_id]" class="width-100" onfocusin="inputLabelOnFocus(this)" onfocusout="inputLabelOutFocus(this)">
			...
			<div class="table-responsive">
				<table class="table table-border width-100">
					<tr>
						...
						<th style="width: 150px; text-align: center;">Quản lý</th>
					</tr>
					@php
					...
					@foreach($getCashFlow as $r)
						<tr>
							...
							<td>
								@if( $r->customer_id )
									<div>
										{!! user("name_color", $r->customer_id) !!}
									</div>
									<div style="color: gray; font-size: 13px">
										ID: {!! $r->customer_id !!} ({!! $r->users_email !!})
									</div>
									<div title="Số dư iện tại của tài khoản" style="color: gray; font-size: 12px">
										SD hiện tại: {{ number_format($r->user_money) }}
									</div>
								@endif
								<div>
									{{ $r->note }}
							...
							<td class="center">
								@if( empty($r->creator_id) )
									@if($r->status == 0)
										<span class="label-warning">Chờ duyệt</span>
									@elseif($r->status == 1)
										<span class="label-success">Đã duyệt</span>
									@endif
									@if($r->accept_user_id)
										<div style="color: gray; font-size: 12px">
											{!! user('name', $r->accept_user_id) !!}
										</div>
									@endif
								@endif
							</td>
							<td class="center">
								@if( empty($r->deleted_at) )
									<div class="cash_flow-manager" data-id="{{ $r->id }}">
										...
										@if( permission('admin') )
											<span class="link btn-danger btn-sm" title="Xóa vĩnh viễn {{ ( $r->customer_id ? '& trừ tiền tài khoản' : 'phiếu thu') }}?" data-action="delete">
